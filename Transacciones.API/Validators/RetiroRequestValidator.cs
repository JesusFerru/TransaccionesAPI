...
{
    public class RetiroRequestValidator : AbstractValidator<RetiroRequest>
    {
        public RetiroRequestValidator()
        {
            RuleFor(x => x.CuentaId)
                .GreaterThan(0).WithMessage("El ID de la cuenta debe ser válido.");

            RuleFor(x => x.Monto)
                .GreaterThan(0).WithMessage("El monto del retiro debe ser mayor a 0.");
